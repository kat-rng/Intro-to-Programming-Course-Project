function convertToCalc()
    ...
    for j = 1:l
        lines(j, :) = [allLines(j).lnID, allLines(j).material.matID, allLines(j).point1.ptID, allLines(j).point2.ptID, allLines(j).material.density, allLines(j).angleInit, allLines(j).area, allLines(j).lengthInit, allLines(j).material.youngsModulus, allLines(j).material.shearModulus];
    end
end